...
import { DEMO_MARKET_OPTIONS } from "@/config/demoSelections";

export interface DemoFilterCriteria {
  market: string;
  side: "over" | "under";
  line: number;
  minOdds: number;
...
interface DemoFilterizerPanelProps {
  onApplyFilters: (filters: DemoFilterCriteria) => void;
  onClearFilters: () => void;
  isActive: boolean;
}

export function DemoFilterizerPanel({ onApplyFilters, onClearFilters, isActive }: DemoFilterizerPanelProps) {
  ...
  const currentMarketOption = DEMO_MARKET_OPTIONS.find(m => m.id === selectedMarket);

  const handleMarketSelect = (marketId: string) => {
    setSelectedMarket(marketId);
    const market = DEMO_MARKET_OPTIONS.find(m => m.id === marketId);
    if (market) {
    ...
    <Card className="p-6 bg-card/50 backdrop-blur-sm border-primary/20">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold">Demo Filterizer</h3>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {DEMO_MARKET_OPTIONS.map((market) => (
              <Button
                key={market.id}
