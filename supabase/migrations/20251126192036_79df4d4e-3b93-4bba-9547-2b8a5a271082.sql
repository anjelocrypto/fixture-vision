-- Fix ambiguous column reference in try_use_feature function
-- Root cause: "remaining_uses" exists both as table column and RETURNS TABLE output parameter
-- This causes PostgreSQL error 42702 when function is called
  ...
  IF cur_remaining > 0 THEN
    UPDATE public.user_trial_credits utc
       SET remaining_uses = utc.remaining_uses - 1,
           updated_at = now()
     WHERE utc.user_id = uid
     RETURNING utc.remaining_uses INTO cur_remaining;
