-- Update pipeline_health_check view to use 48-hour window for coverage calculations
-- This is the 48-hour horizon enforcement for pipeline monitoring

DROP VIEW IF EXISTS pipeline_health_check;

CREATE VIEW pipeline_health_check AS
WITH stats_summary AS (
  SELECT 
    COUNT(DISTINCT tm.team_id) AS total_teams,
    COUNT(DISTINCT sc.team_id) AS fresh_stats,
    ROUND(100.0 * COUNT(DISTINCT sc.team_id) / NULLIF(COUNT(DISTINCT tm.team_id), 0), 1) AS coverage_pct
  FROM (
    SELECT DISTINCT (teams_home->>'id')::INT AS team_id FROM fixtures
    WHERE timestamp >= EXTRACT(EPOCH FROM NOW()) 
      AND timestamp <= EXTRACT(EPOCH FROM NOW() + INTERVAL '48 hours')
    UNION
    SELECT DISTINCT (teams_away->>'id')::INT AS team_id FROM fixtures
    WHERE timestamp >= EXTRACT(EPOCH FROM NOW()) 
      AND timestamp <= EXTRACT(EPOCH FROM NOW() + INTERVAL '48 hours')
  ) tm
  LEFT JOIN stats_cache sc ON sc.team_id = tm.team_id 
...
SELECT 
  ...
  CASE
    ...
    WHEN ss.coverage_pct < 70 THEN '❌ CRITICAL (LOW COVERAGE)'
    WHEN EXTRACT(EPOCH FROM (NOW() - lb.last_warmup_optimizer)) / 60 > 40 THEN '⚠️ DEGRADED (WARMUP DELAYED)'
    WHEN ss.coverage_pct < 90 THEN '⚠️ DEGRADED (COVERAGE BELOW 90%)'
