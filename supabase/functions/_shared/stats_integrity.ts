// Stats Integrity Helper - shared validation for Filterizer & Ticket Creator
// Ensures both tools NEVER use broken/missing stats
...
/**
 * Validates that both teams in a fixture have reliable stats
 * Returns isValid=false if either team has:
 * - No stats_cache entry
 * - sample_size < 3
 * - An active CRITICAL goals violation
 */
export async function validateFixtureStats(
  ...
  // Check for active critical violations on goals
  ...
  const result: StatsValidation = {
    ...
    homeTeam: {
      ...
      hasCriticalViolation: !!homeViolation && homeViolation.severity === 'critical'
    },
    awayTeam: {
      hasCache: !!awayCache,
      sampleSize: awayCache?.sample_size || 0,
      hasCriticalViolation: !!awayViolation && awayViolation.severity === 'critical'
    }
  };
  ...
  if (!homeCache) {
    ...
    result.isValid = false;
    result.reason = `Home team (${homeTeamId}) has sample_size=${homeCache.sample_size} (need ${MIN_SAMPLE_SIZE}+)`;
  } else if (homeViolation?.severity === 'critical') {
    result.isValid = false;
    result.reason = `Home team (${homeTeamId}) has CRITICAL goals violation`;
  }

  // Validate away team (only if home passed)
  if (result.isValid) {
    if (!awayCache) {
      result.isValid = false;
      ...
      result.isValid = false;
      result.reason = `Away team (${awayTeamId}) has sample_size=${awayCache.sample_size} (need ${MIN_SAMPLE_SIZE}+)`;
    } else if (awayViolation?.severity === 'critical') {
      result.isValid = false;
...
export async function validateFixturesBatch(
  ...
  // Fetch all active critical violations at once
  const { data: violations } = await supabase
    .from("stats_health_violations")
    .select("team_id, severity, metric")
    .in("team_id", Array.from(teamIds))
