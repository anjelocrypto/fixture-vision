...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Auth check (same as other admin functions) - NO .single() on scalar RPCs!
    ...
    if (cronKeyHeader) {
      ...
      if (keyError) {
        console.error("[turbo] get_cron_internal_key error:", keyError);
      } else {
    ...
    if (!isAuthorized && authHeader) {
      if (authHeader === `Bearer ${serviceRoleKey}`) {
        ...
        if (anonKey) {
          ...
          if (wlError) {
            console.error("[turbo] is_user_whitelisted error:", wlError);
    ...
    if (!isAuthorized) {
      console.error("[turbo] Authorization failed - no valid credentials");
